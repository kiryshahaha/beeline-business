# Документация проекта

| Файл / папка | Что прочитать |
| --- | --- |
| [OPERATIONS.md](OPERATIONS.md) | Поддерживаемый режим доставки уведомлений, журнал операций, общий замок, хранение и очистка, резервное копирование |
| [DATA_EXCHANGE.md](DATA_EXCHANGE.md) | Структура CSV/XLSX, импорт, экспорт, ID, безопасность и примеры |
| [ACCESS_CONTROL.md](ACCESS_CONTROL.md) | Матрица чтения и изменения данных по ролям, сессии и публичная ссылка календаря |
| [AUDIT_2026-09-18.md](AUDIT_2026-09-18.md) | Области проверки, подтверждённые проблемы и результаты тестов |
| [PLANNING_VERIFICATION.md](PLANNING_VERIFICATION.md) | Реализованная интеграция planner, результаты проверок и ограничения окружения |
| [PLANNING_POLICY.md](PLANNING_POLICY.md) | Контракт кейса T01, действующие правила, сохранение параметров и границы проверок |
| [PLANNER_INTEGRATION.md](PLANNER_INTEGRATION.md) | Исходный технический аудит и проект интеграции; актуальный контракт — в README модуля planning |

Контракт работающего API также доступен в `/docs` и `/openapi.json` backend.
Локальные README внутри модулей объясняют соседние файлы и ограничения моделей.

При реализации трёх дополнений учитывались предоставленные материалы
`3. Билайн Бизнес.pdf` и `transcript_clean.txt`: четыре вида транспорта,
порядок посещений, время прибытия, планирование по дням, синтетические сценарии.
Полный кейс шире этих дополнений. Приоритеты заявок, автоматическое перепланирование,
карта и интеграция solver с UI не добавляются автоматически только из-за упоминания
в этих материалах.
