...
class OfficesApiTests(DatabaseTestCase):
    def setUp(self):
        ...
        city_id = self.connection.execute(text("INSERT INTO cities (name) VALUES ('Город') RETURNING id")).scalar_one()
    ...
    def _auth_headers(self, user) -> dict:
        response = self.client.post("/api/v1/auth/login", json={"username": user.username, "password": "Password123!"})
    ...
    def test_create_office_duplicate_name(self):
        headers = self._auth_headers(self.observer)
        self.client.post("/api/v1/offices/", json={"name": "Новый Офис", "location_id": self.location_id}, headers=headers)
        response = self.client.post("/api/v1/offices/", json={"name": "Новый Офис", "location_id": self.location_id}, headers=headers)
    ...
    def test_create_office_invalid_location(self):
        headers = self._auth_headers(self.observer)
        response = self.client.post("/api/v1/offices/", json={"name": "Новый Офис", "location_id": 9999}, headers=headers)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()["detail"], "Локация с таким ID не найдена")

    def test_list_offices(self):
        headers = self._auth_headers(self.observer)
        self.client.post("/api/v1/offices/", json={"name": "Офис 1", "location_id": self.location_id}, headers=headers)
        self.client.post("/api/v1/offices/", json={"name": "Офис 2", "location_id": self.location_id}, headers=headers)
    ...
    def test_worker_can_list_offices(self):
        ...
        self.client.post("/api/v1/offices/", json={"name": "Офис 1", "location_id": self.location_id}, headers=headers_obs)
