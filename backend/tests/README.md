...
| `test_worker_equipment_api.py` | T08: комплект и резерв, выдача, однократное списание, компенсация, отмена, возврат, переназначение, роли, гонка за последнюю единицу, миграция 0016 |
| `test_analytics_*` | T16: текущая очередь отдельно от периода, граница суток по Москве при БД в UTC, правка закрытой заявки не завершение; загрузка по смене и маршруту, отсутствие на дату; лента из доменных событий с автором назначения, снятия и прямого SQL |
| `test_schedule_api.py`, `test_schedule_timeline.py` | Интервалы смен, заявки на сутки, фильтр офиса и роли; план дня в модели смены планировщика, ночная смена, отсутствие на дату, конфликты маршрута, очередь без исполнителя по участку |
| `test_notifications*`, `test_firebase_gateway.py` | Очередь, WebSocket, токены и граница SDK Firebase |
| `test_operability_t23.py` | T23: журнал без секретов и персональных данных, `X-Request-ID`, ограниченное ожидание общего замка и 503, догонка WebSocket по `last_event_id` без дублей, отказ процесса без роли доставки, очистка просроченных предпросмотров с сохранением применённых |
| `test_calendar_feed_api.py` | Файл iCalendar, хеш токена, перевыпуск, отзыв и роли; снятая заявка как CANCELLED без адреса, горизонт, отметка завершения, более 1000 событий без потери ближайшего будущего |
| `test_worker_line_status_api.py` | Снятие и возврат инженера, освобождение `planned`, транзакция, роли, миграция и планирование |
| `test_execution_*` | Канонический жизненный цикл заявки, события рабочего дня, ревизии, повтор команд и расход оборудования |
| `test_day_plan_revisions_t09.py` | T09: единственная текущая ревизия участка и дня, неизменная история и геометрия, точный diff, пометка маршрутов и устаревание предложения после новой заявки в том же участке |
...
test_planning_reasons.py — причины precheck и проверки выброшенных визитов на малых задачах без БД.
T10 в test_planning_api.py: пустой план 201 и запрет apply, факторы визитов, навык,
которого нет ни у кого, заполненные маршруты с оценкой доп. ресурса, пропущенное решателем
место, решатель без решения как ошибка инфраструктуры и чтение планов до T10.
