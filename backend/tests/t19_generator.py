...
def generate_t19_dataset(variant="base", seed=1900):
    ...
    for i in range(1, 4):
        ...
        for b in range(1, 40): # locations per area
            b_id = i * 100 + b
            add("buildings", id=b_id, city_id=i, street_id=i, service_area_id=100 + i, number=str(b))
            add("entrances", id=b_id, building_id=b_id, number="1")
            
            lat = round(55.75 + b_id * 0.0001, 6)
            lon = round(37.61 + b_id * 0.0001, 6)
            add("locations", id=b_id, building_id=b_id, latitude=lat, longitude=lon, entrance_id=b_id)

        # 1 Office per area
        add("offices", id=i, location_id=i * 100 + 1, name=f"Office {i}")
        # 1 Brigade per area
        add("brigades", id=i, division_id=i, name=f"Brigade {i}")
    ...
    transport_profiles = [TransportType.CAR, TransportType.WALKING, TransportType.BICYCLE, TransportType.PUBLIC_TRANSPORT]
    for area_idx in range(1, 4):
        for w_idx in range(1, 13):
            user_id = add("users", username=f"worker_{area_idx}_{w_idx}", role="worker")["id"]
            trans = transport_profiles[w_idx % 4]
            add(
                "workers", 
                user_id=user_id, 
                first_name=f"W{w_idx}", 
                last_name=f"A{area_idx}",
    ...
    add("work_types", id=1, name="EM", category=TicketCategory.EMERGENCY.value, default_priority=1)
    add("work_type_planning_rules", work_type_id=1, service_duration_source="fixed", fixed_duration_minutes=60)
    add("work_type_required_skills", work_type_id=1, skill_id=1) # requires Copper
    
    add("work_types", id=2, name="CONN", category=TicketCategory.CONNECTION.value, default_priority=2)
    add("work_type_planning_rules", work_type_id=2, service_duration_source="fixed", fixed_duration_minutes=90)
    add("work_type_required_skills", work_type_id=2, skill_id=2) # requires Fiber
    
    add("work_types", id=3, name="REP", category=TicketCategory.REPAIR.value, default_priority=3)
    add("work_type_planning_rules", work_type_id=3, service_duration_source="ticket_estimate")
    
    add("work_types", id=4, name="ADD", category=TicketCategory.ADDITIONAL.value, default_priority=4)
    add("work_type_planning_rules", work_type_id=4, service_duration_source="fixed", fixed_duration_minutes=30)
    ...
    for wt_id, count in distribution:
        for _ in range(count):
            ...
            if ticket_idx == 1:
                # Emergency at 12:17
                comments.append({"text": "Urgent leak", "created_at": datetime(2030, 1, 15, 12, 17, tzinfo=UTC)})
            elif ticket_idx == 2:
            ...
            ticket = add(
                ...
                work_type="WT",
                work_type_id=wt_id,
                category=TicketCategory(wt_id).name.lower() if wt_id <= 4 else TicketCategory.REPAIR.value,
                priority=1,
                received_at=stamp,
            ...
            for comment in comments:
                add("ticket_comments", ticket_id=ticket_idx, author_id=1, text=comment["text"], created_at=comment["created_at"])
