...
class CommittedDatabaseTestCase(DatabaseTestCase):
    ...
    def setUp(self):
        with self.engine.begin() as connection:
            ...
            connection.execute(
                text(
                    "TRUNCATE brigade_members, brigades, offices, notification_events, push_subscriptions, "  # noqa: E501
                    "ticket_comments, "
                    "ticket_assignments, tickets, refresh_tokens, worker_skill_assignments, "
                    "worker_skills, workers, users, locations, entrances, buildings, streets, "
                    "districts, cities"
                )
            )
