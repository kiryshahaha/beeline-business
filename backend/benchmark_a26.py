    ...
    async def build_route_matrix(self, sources, targets, mode):
        ...
        if self.telemetry is not None:
            self.telemetry.record_operation(
                "matrix", mode, cells=len(sources) * len(targets), source="mock"
            )  # noqa: E501
        return RouteMatrixResult(
            cells=[
                [RouteMatrixCell(distance_meters=1000, duration_seconds=600) for _ in targets]
                for _ in sources
            ]  # noqa: E501
        )
...
async def measure(tickets_count, workers_count):
    ...
    epoch = datetime(2026, 9, 26, 9, tzinfo=UTC)
    prepared = {
        "epoch": epoch,
        "horizon": 120,
    ...
    for i in range(workers_count):
        ...
        prepared["locations"][loc_id] = {
            "longitude": 37.0 + (i % 3) * 0.1,
            "latitude": 55.0 + (i % 3) * 0.1,
        }  # noqa: E501
        ...
        with patch("app.modules.planning.service.Session"):
            with patch.object(
                ...
                return_value={
                    ...
                    "planning_policy": prepared["policy"].model_dump(mode="json"),
                },
            ):  # noqa: E501
