...
роли (`observer`, `foreman`, `worker`), профили исполнителей, бригады, JWT-авторизация,
...
- **Бригады:**
  - `POST /api/v1/brigades` — создать бригаду с бригадиром и составом (только `observer`);
...
Таблица `brigades` хранит название и `foreman_id`; `brigade_members` связывает бригаду
с пользователем-исполнителем. `POST /api/v1/brigades` принимает `name`, `foreman_id` и
`worker_ids`. Запрос `PUT /api/v1/brigades/{id}/members` заменяет `foreman_id` и весь
...
├── migrations/                # последовательные изменения схемы БД (0001–0006)
