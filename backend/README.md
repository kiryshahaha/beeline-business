...
справочник видов работ с нормативами времени (миграции `0001`–`0015`). Дополнительно
реализованы тип транспорта исполнителя, история маршрутов в GeoJSON, единый импорт/экспорт
всех предметных таблиц CSV/XLSX, выгрузка отфильтрованных заявок для отчётности
и личный календарь заявок исполнителя. Наблюдатель может снять инженера с линии
и вручную вернуть его в список доступных сотрудников.
...
- **Пользователи, роли и профили:**
  ...
  - `PUT /api/v1/workers/{worker_id}/line-status` — снять инженера с линии либо вернуть
    его обратно; доступно только роли `observer`.
- **Офисы:**
...
- **Складской учет и оборудование:**
  ...
  - `GET|PUT /api/v1/offices/{office_id}/kit-reserve[/{appliance_id}]` — норма резерва комплекта инженера офиса (изменение — `observer`);
  - `GET /api/v1/workers/{worker_id}/equipment?date=` — оборудование на руках, закреплённое и свободное, комплект к выдаче и нехватка на складе;
  - `POST /api/v1/workers/{worker_id}/equipment/issue` и `/return` — выдача комплекта дня и явный возврат (только `observer`, идемпотентны по `operation_key`);
  - `POST /api/v1/tickets/{ticket_id}/equipment/restore` — вернуть списанное после переоткрытия заявки (только `observer`);
  - `GET /api/v1/equipment/operations` — журнал движения оборудования (только `observer`).
- **Бригады:**
...
- Поле `worker_profile.is_on_line` показывает доступность инженера для новых назначений
  и планирования; новый профиль получает значение `true`.
...
2. **Складские остатки офисов (`appliance_stocks`):**
   ...
   - Доступный для новых заявок остаток рассчитывается динамически в реальном времени:
     ...
     количество позиций в заявках со статусами `planned` и `in_progress`, ещё не выданных инженеру.
...
3. **Оборудование в заявках (`ticket_appliances`):**
   ...
   - Состав можно менять, пока оборудование лежит в офисе; выданное или списанное — только после возврата.
   - При переводе заявки в статус `COMPLETED` расходные материалы списываются ровно один раз:
     с рук инженера, если комплект выдан, иначе со склада офиса. Нехватка — ошибка `409`,
     а не скрытый ноль. Инструменты (`TOOL`) не списываются.
   - При отмене заявки (`WONT_FIX`) резерв офиса освобождается, а уже выданное остаётся у
     инженера свободным запасом до явного возврата.
4. **Оборудование на руках (T08):** комплект дня, норма резерва офиса, выдача, возврат,
   компенсация после переоткрытия и журнал операций описаны в
   [README модуля](app/modules/appliances/README.md).

## Технологии и структура

CPython 3.12, FastAPI, SQLAlchemy 2, PostgreSQL 17, Alembic, Argon2, PyJWT.
PostgreSQL 15 или новее нужен для уникальных индексов `NULLS NOT DISTINCT`:
...
backend/
├── app/
│   ├── main.py                # приложение FastAPI, /health, /ready и OpenAPI
│   ├── core/
...
│       │   ├── router.py       # /api/v1/users/*, /api/v1/workers/* и справочник навыков
...
├── migrations/                # последовательные изменения схемы БД (0001–0015)
...
| `workers` | `user_id`, `workshift_start`, `workshift_end`, `transport_type`, `is_on_line` | Профиль выездного специалиста, смена, транспорт и доступность для назначений |
...
| Папка | Запросы |
| --- | --- |
| `01-system` | `GET /health`, `GET /ready` |
...
Для демонстрации Compose создаёт отдельный экземпляр PostgreSQL 17 и запускает
миграции перед backend. Полный сценарий с генерацией секретов и удалением только
тестовых контейнеров описан в руководстве runtime. Тесты используют отдельную БД
с именем, оканчивающимся на _test, и собственные временные схемы.

## Миграции

Миграции применяются при старте Compose backend. Текущую ревизию можно проверить
в работающем проекте командой:

docker compose -p beeline-t20 exec backend python -m alembic current

GET /ready отвечает 200, если PostgreSQL доступен, Alembic находится на текущей
голове, а planner отвечает на /health. GET /health проверяет liveness API. Оба
маршрута не обращаются к Geoapify.
...
Сторонние actions закреплены по SHA: checkout 7.0.1, setup-python 7.0.0,
setup-node 7.0.0 и upload-artifact 4.6.2. Backend job использует Node 24
для Bruno; planner и API проверяются на Python 3.12 и PostgreSQL 17.
...
Наблюдатель меняет доступность инженера запросом:

```http
PUT /api/v1/workers/42/line-status
Authorization: Bearer <observer-access-token>
Content-Type: application/json
...
Ответ содержит ID инженера, новый статус и заявки, которые освободились:

```json
{"worker_id":42,"is_on_line":false,"released_ticket_ids":[17,21]}
```

При значении `false` сервер удаляет назначения этого инженера со всех заявок
в статусе `planned`. Если у заявки больше нет исполнителей, плановые даты очищаются;
при совместном назначении даты сохраняются. Заявки `in_progress`, `completed` и
`wont_fix`, а также сохранённые маршруты остаются без изменений.

Возврат выполняется тем же запросом с `{"is_on_line":true}`. Сервер открывает
инженера для новых назначений и планирования, но не восстанавливает освобождённые
заявки. Повторная передача текущего значения безопасна и возвращает пустой
`released_ticket_ids`. Назначение снятого с линии инженера через
`PUT /api/v1/tickets/{id}/assignees` получает `422`.

Смена статуса, удаление плановых назначений и очистка дат проходят в одной
транзакции под общей блокировкой операций планирования. Статус виден в профиле
пользователя и в ответе `GET /api/v1/schedule`. Планирование исключает такого
инженера с причиной `worker_offline`.
...
Bruno покрывает HTTP-маршруты в папках `05-auth`–`22-worker-equipment`.
