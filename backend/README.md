...
- **Маршрутизация:**
  - `POST /api/v1/routes` — рассчитать автомобильный маршрут между двумя координатами; доступно роли `observer`.
...
├── bruno/                    # коллекция HTTP-запросов и проверок Bruno (01–11)
└── README.md
```
...
`POST /api/v1/routes` строит один автомобильный маршрут через Geoapify Routing API.
Ручка принимает координаты начала и конца, возвращает дистанцию в метрах, длительность
в секундах и GeoJSON-геометрию `MultiLineString`. Вызывать её может только `observer`: такой доступ не
...
Поле `mode` пока принимает только `drive`. Входные точки используют поля
...
`GeoapifyRoutingClient.build_route_matrix(...)` — внутренняя функция для
будущей связи основного сервиса с `planner`. Она вызывает Route Matrix API,
принимает списки `GeoPoint`, возвращает матрицу дистанций и времени, а пары
без дорожного пути оставляет `null`. В запросе Matrix Geoapify требует порядок
`[longitude, latitude]`. Один синхронный вызов содержит от одной до 1 000 ячеек;
крупные матрицы понадобится разбивать на части. Функция не зарегистрирована как
HTTP-ручка. Перед передачей матрицы в solver нужно согласовать перевод секунд
в минуты и правило для недостижимых пар.
...
| `09-routing` | Маршрут наблюдателя через Geoapify и проверка запрета для исполнителя |
...
Bruno покрывает все новые HTTP-маршруты в папках `05-auth`–`09-routing`.
...
которые сохраняют запросы входа из `05-auth`. Запуск «Наблюдатель строит маршрут»
...
4. Хранение планов, чтение готового маршрута назначенным исполнителем и связь
   Geoapify Matrix с планировщиком.
