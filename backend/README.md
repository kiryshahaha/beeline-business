...
роли (`observer`, `worker`), профили исполнителей, справочник навыков, JWT-авторизация,
назначения, комментарии и уведомления (миграции `0001`–`0004`).
...
- **Пользователи и профили:**
  ...
  - `GET /api/v1/users` — список всех пользователей (доступно всем авторизованным пользователям);
  - `POST /api/v1/users` — создать пользователя (только для роли `observer`);
  - `GET /api/v1/users/{id}` — получить данные пользователя по ID (доступно всем авторизованным пользователям);
...
- **Заявки и система:**
  ...
  - `GET /api/v1/tickets` — получить страницу заявок с фильтрами;
...
Реализованы основа приложения, модель заявки, адресный справочник и миграции PostgreSQL.
Для заполнения БД демонстрационными данными есть `seed_demo.py`.
Доступны три операции HTTP API:

- `POST /api/v1/tickets` — создать заявку;
- `GET /api/v1/tickets` — список заявок с фильтрами и пагинацией;
- `GET /api/v1/tickets/{id}` — получить одну заявку с адресом и координатами.

Служебный `GET /health` сохранён. Изменение и удаление заявок, API адресного
справочника, авторизация и планировщик на этом этапе не реализованы.

В `bruno/` находится коллекция ручных HTTP-запросов с проверками ответов:
фильтры списка, пагинация, создание/чтение заявки и ожидаемые ошибки.
Инструкция — в разделе «Запросы в Bruno» ниже.

Принятые совместно решения:

- Пароли хешируются алгоритмом **Argon2** (`argon2-cffi`).
- Токены формируются по стандарту **JWT** (`PyJWT`).
- Роли: `observer` (наблюдатель/диспетчер) и `worker` (выездной специалист).
- Открытая регистрация отключена: новых пользователей создаёт только наблюдатель.
- Список всех пользователей доступен любому авторизованному пользователю.
...
│       ├── users/
│       │   ├── enums.py        # роли UserRole (observer, worker)
...
├── migrations/                # последовательные изменения схемы БД (0001, 0002, 0003)
├── tests/                     # проверки приложения, безопасности, токенов и БД
├── seed_demo.py               # заполнение демоданными (заявки, навыки, наблюдатели, исполнители)
...
│           ├── enums.py      # статусы
│           ├── schemas.py    # поля запроса, проверки и ответ
│           ├── router.py     # POST, GET списка / по ID, параметры и HTTP-коды
│           ├── service.py    # создание, чтение, общая сборка ответа и транзакция
│           └── repository.py # SQL-запросы
├── migrations/               # последовательные изменения схемы БД
├── tests/                    # проверки приложения и ограничений БД
├── bruno/                    # коллекция HTTP-запросов, окружение Local и проверки ответов
...
| Таблица | Основные поля | Что хранит |
| --- | --- | --- |
| `users` | `id`, `name`, `surname`, `lastname`, `username`, `password_hash`, `role` | Учётная запись пользователя, роль `observer` или `worker`, Argon2-хеш |
...
| `05-auth` | Вход наблюдателя и двух исполнителей; сохранение access-токенов и ID |
| `06-ticket-workflow` | Очистка и установка назначений, смена статуса, проверки прав и отсутствующих записей |
| `07-comments` | Комментарии наблюдателя и исполнителя, чтение ленты, проверки `403` и `422` |
| `08-notifications` | История событий двух ролей, регистрация, перенос и удаление Firebase-токена, проверки `401` и `404` |

Всего **50 запросов**. В `02-ticket-list/folder.bru` находятся общие проверки
...
заметок. Наблюдатель видит комментарии каждой карточки, исполнитель получает доступ
только после назначения. Текст содержит от 1 до 4000 символов после удаления пробелов
по краям; автор и время берутся на сервере.
...
на заявку. Наблюдатель тоже может редактировать только собственные комментарии.
Комментарий должен принадлежать заявке из URL. Сервер возвращает `200` и полный
объект `CommentRead`; ID, автор, дата создания и порядок в ленте сохраняются.
История правок и отдельная дата редактирования не сохраняются.
...
Bruno покрывает все новые HTTP-маршруты в папках `05-auth`–`08-notifications`.
...
1. API адресного справочника; создание места выполнения делает другой участник команды.
2. Правила переходов статусов и обновления расписания.
3. История переносов: изменения расписания и объясняющие их комментарии должны сохраняться.
4. Квалификации, оборудование, транспорт, SLA и планировщик маршрутов.
